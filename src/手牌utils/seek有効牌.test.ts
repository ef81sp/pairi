import { generateExtractResult七対子, generate牌List } from "../utils/testUtils"
import { seek有効牌七対子 } from "./seek有効牌.mjs"
import { ExtractResult七対子 } from "./手牌utils.type.mjs"

describe("seek有効牌七対子", () => {
  test("かぶりなし一向聴の場合、restに残っている牌", () => {
    const extractResult七対子: ExtractResult七対子 = generateExtractResult七対子(
      [
        ["1m", "1m"],
        ["2m", "2m"],
        ["3m", "3m"],
    ...
    expect(result).toEqual(generate牌List(["1p", "4s", "6z"]))
  })
